...
- 회원가입 - NAVER SENS (문자 메시지 인증) 및 정규식으로 유효성 체크

- Spring Security 자체로그인, OAuth2 카카오 / 네이버 간편 로그인

- 거래게시판 CRUD : 
페이징은 QueryDSL으로 동적으로 검색 필터 적용.
상세보기와 수정 삭제는 useState로 관리. 
글쓰기와 수정시 대표이미지 및 이미지 순서를 바꿀수 있게 React 라이브러리 dnd-kit 및 sortable Component로 구현.

- 거래게시판 지도 거래 장소 확대 및 축소 기능 : GeoJson과 React 의 turf 라이브러리로 원하는 지역 범위 설정

- 카카오 지도 API로 위치 인증, 거래 장소 잡기 기능 : Kakao Map API 공식 참조 사이트를 통해 각종 마커 및 마커의 움직임 , 마커에 따른 행정 중심지입력 등 다양하게 활용하여 구현
